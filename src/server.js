...
// NIFTY 50 lot size (65 from Jan 2026 expiries, per NSE Oct 2025 circular). Update here if NSE revises it.
...
function analyse(chain, expiryDate) {
  ...
  if (tradeLegs && (strategy === 'LONG_CALL' || strategy === 'LONG_PUT') && tradeLegs.buyLeg) {
    ...
    const legDelta = tradeLegs.legDelta || 0.5;
    let slPrem, tgtPrem, slBasis, targetBasis;
    if (strategy === 'LONG_CALL') {
      // Stop: thesis breaks if spot breaches the PE support wall
      slBasis = 'PE wall ' + peWall;
      slPrem = entryPrem - legDelta * Math.max(0, spot - peWall);
      // Target: nearer of EM high / CE wall above spot
      const ups = [{ v: emHigh, l: 'EM high ' + emHigh }, { v: ceWall, l: 'CE wall ' + ceWall }]
        .filter(c => c.v > spot).sort((a, b) => a.v - b.v);
      const tgt = ups[0] || { v: emHigh, l: 'EM high ' + emHigh };
      targetBasis = tgt.l;
      tgtPrem = entryPrem + legDelta * Math.max(0, tgt.v - spot);
    } else {
      // Stop: thesis breaks if spot breaches the CE resistance wall
      slBasis = 'CE wall ' + ceWall;
      slPrem = entryPrem - legDelta * Math.max(0, ceWall - spot);
      // Target: nearer of EM low / PE wall below spot
      const downs = [{ v: emLow, l: 'EM low ' + emLow }, { v: peWall, l: 'PE wall ' + peWall }]
        .filter(c => c.v < spot).sort((a, b) => b.v - a.v);
      const tgt = downs[0] || { v: emLow, l: 'EM low ' + emLow };
      targetBasis = tgt.l;
      tgtPrem = entryPrem + legDelta * Math.max(0, spot - tgt.v);
    }
    slPrem = Math.max(0.05, slPrem);
    const riskPts = entryPrem - slPrem;
    const rewardPts = tgtPrem - entryPrem;
    tradeLegs.single = true;
    tradeLegs.entry = entryPrem.toFixed(2);
    tradeLegs.stopLoss = slPrem.toFixed(2);
    tradeLegs.target = tgtPrem.toFixed(2);
    tradeLegs.slBasis = slBasis;
    tradeLegs.targetBasis = targetBasis;
    tradeLegs.riskRupees = pointsToRupees(riskPts);
    tradeLegs.rewardRupees = pointsToRupees(rewardPts);
    tradeLegs.planRRR = riskPts > 0 ? (rewardPts / riskPts).toFixed(2) : 'N/A';
  }
