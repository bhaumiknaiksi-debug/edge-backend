...
async function fetchLiveChain(jwt) {
 ...
 "/rest/secure/angelbroking/market/v1/optionGreek?exchange=NFO&symboltoken=99926000&expiry "GET", null, jwt
 );
 if (!res?.data || !Array.isArray(res.data)) throw new Error("Empty chain: " + JSON.stringif const strikeMap = {};
 let spot = 0;
...
async function runAnalysis() {
 ...
 return { timestamp: new Date().toISOString(), spot: chain.spot, expiry: chain.expiry, sr, p}
// ── Broadcast ──
...
async function pollAndBroadcast() {
 ...
 console.log(`SPOT: ${result.spot} | BIAS: ${result.sentiment.bias} | PCR: ${result.pcr.ov broadcast({ type: "analysis", ...result });
 } catch(err) {
...
app.post("/login", async (req, res) => {
 ...
 return res.status(401).json({ success: false, message: loginRes?.message || "Login fail }
...
// ── REST ──
app.get("/", (req, res) => res.json({ status: "EDGE Engine online", phase: 3, live: !!sessionapp.get("/analysis", async (req, res) => {
