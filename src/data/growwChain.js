...
function getNearestExpiry() {
  // Compute IST offset manually (UTC+5:30) to avoid locale string arithmetic bugs
  var now     = new Date();
  var istMs   = now.getTime() + (5 * 60 + 30) * 60 * 1000;
  var ist     = new Date(istMs);
  var day     = ist.getUTCDay();   // 0=Sun 1=Mon ... 4=Thu
  var hours   = ist.getUTCHours();
  var minutes = ist.getUTCMinutes();
  var mins    = hours * 60 + minutes;

  var daysToThursday = (4 - day + 7) % 7;
  // If today IS Thursday and market has closed (after 3:30 PM IST), use next week
  if (daysToThursday === 0 && mins >= 930) daysToThursday = 7;
  // If daysToThursday is 0 it means today is Thursday (before close)
  // All other days: add correct days
...
async function fetchGrowwChain() {
  ...
  // Log full payload structure to diagnose
  var payload = res.body.payload;
  var payloadKeys = Object.keys(payload || {});
  console.log("Groww payload keys: " + payloadKeys.join(", "));
  console.log("Groww raw: underlying_ltp=" + payload.underlying_ltp +
    " strikes_count=" + Object.keys(payload.strikes || payload.optionChain || payload.data || {}).length);
